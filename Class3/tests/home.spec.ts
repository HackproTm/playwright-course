...
test.describe('Automation Exercise', ()=>{
    test('Register user', async ({ page }) => {
        ...
        await page.getByPlaceholder('Name').fill('Hackpro')
        await page.locator('[data-qa="signup-email"]').fill('hackpro@yopmail.com')
        //Click en boton signup
        await page.getByRole('button', { name: 'Signup' }).click()
        //Click en MR. o MRs.
        await page.getByRole('radio', { name: 'Mr.' }).click()
        //Rellenar password
        await page.locator('#password').fill('hackprotm')
        ...
        await page.locator('#first_name').fill('Hackpro')
        await page.locator('#last_name').fill('Team')
